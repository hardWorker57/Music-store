...
const Goods_list = () => {
  ...
  let title = useRef(null); let dash = useRef(null); let card = useRef(null);
  ...
  useEffect(() => {
    ...
    gsap.to(dash.current, { width: '100%', opacity: 1, duration: .6, delay: .4, scrollTrigger: { trigger: dash.current } });
    
    gsap.from('.card', {opacity: 0, y: 100, scale: 0.5})
    gsap.to('.card', {opacity: 1, y: 0, scale: 1, ease: true, stagger: 0.15, duration: 1, delay: .8,  scrollTrigger: { trigger: '.card' }});
